...
function Test-Command {
    ...
    if ($invalidAction -or $invalidName) { Show-Help exit }
...
function Get-Manifest {
    ...
    try {
        $uri = $Uri ? $Uri : "$BaseUrl/getManifest/$Name"
        $headers = Get-Headers -Private:$Private -NoCache:$NoCache
...
function Invoke-Download {
    ...
    function Invoke-Aria2Download {
        $headers = Get-Aria2Headers -Private:$Private -NoCache:$NoCache
...
function Expand-Path {
    ...
    foreach ($pathMap in $pathsMap.GetEnumerator()) {
        $Path = $Path.Replace($pathMap.key, $pathMap.Value)
    }
    return $Path
}
...
function Write-Manifest {
    ...
    if ($Manifest) {
        Write-Host
        foreach ($manifest in $manifestMap.GetEnumerator()) {
            if ($manifest.Value) {
                Write-FormattedRow -Text $manifest.Key, ": $(Expand-Path -Path $manifest.Value)"
            }
        }
        Write-Host
    }
...
function Invoke-Search {
    ...
    $manifestIndex = Get-Manifest -Uri "$BaseUrl/getIndexManifest" -Private:$Private -NoCache:$NoCache
    if (-not $manifestIndex) {
        Write-FormattedRow -Text "Not found!", ": manifests-index.json" -Color Yellow
        return
    }

    $results = $manifestIndex | Where-Object {
        $_.id -like "*$Name*" -or
        $_.name -like "*$Name*" -or
...
function Show-Manifest {
    ...
    $manifest = Get-Manifest -Name $Name -Private:$Private -NoCache:$NoCache
    if (-not $manifest) { 
        Invoke-Search -Name $Name -Private:$Private -NoCache:$NoCache
        return
    }
